...
public class DocAnalyzer   {
	public static void main(String[] args) { 
		...
		String doc_labels = "C:\\Users\\bgree\\Documents\\capstone\\Eng\\eng-docs-labels.txt";
		String docs = "C:\\Users\\bgree\\Documents\\capstone\\Eng\\eng_clean\\";
		DocAnalyzer docAnalyzer=new DocAnalyzer(6,true);  
	...
	public ArrayList<Tokenizer> tokenizer; // need many because of the threading
	//a list of stopwords
	...
	public DocAnalyzer(int NumberOfProcessors ,Boolean debug) {
		...
		String resultsPath = "C:\\Users\\bgree\\Documents\\capstone\\Results\\Logistic Regression\\2-15-18.txt";

		try {
			FileWriter fstream = new FileWriter(resultsPath, true);
			BufferedWriter out = new BufferedWriter(fstream);
			out.write("English Language Agnostic English, Logistic Regression first run and feature export\n\n"); 
			out.close();
...
//	}
	...
	public void analyzeDocumentDemo(String filename,int isPositive, int core) {		
		try {
            ...
            reader.close();
         // process Content
            for(String finalToken:tokenizer.get(core).tokenize(content)){
	...
	public double[] EvaluateClassifier(LogisticRegressionClassifier Classifier,String probFile){ 
		...
		try {
			Writer   fstream = new OutputStreamWriter(new FileOutputStream(probFile), StandardCharsets.UTF_8);
			BufferedWriter out = new BufferedWriter(fstream);
		for(int i=0;i<testingDocuments.size();++i){
			Document testingDoc=testingDocuments.get(i);
			double testingLabel=testingDoc.getLabel();
			double prob=Classifier.Classify(testingDoc);
			double label=prob>0.5?1:0;  
			out.write(FilenameUtils.removeExtension(new File(testingDoc.getFileName()).getName())+","+prob+"\n");
	...
	public void LoadVocab(String filename)
	{
	 ...
	 public void RunAnalyzerOnOneGroup( String DataDirPath,String labelsFilePath,String[] groupsToRemove,String targetGroup ) {	
			...
			BuildVectorSpaceModel(testingFiles, labels, false);
	        if(testingDocuments.size()==0) {
	        	System.out.println("No testing documents for "  + targetGroup);
	        	return;
	        }
			...
			String groupDir = "C:\\Users\\bgree\\Documents\\capstone\\Eng\\exported-features\\" + targetGroup + "\\";
			...
			if(testingDocuments.size() > 0) {
				String write = "";
				 for(Document d : testingDocuments) {
					 write += d.getFileName() + "\n";
				 }
				 
				 try (Writer writer = new BufferedWriter(new OutputStreamWriter(
			              new FileOutputStream("C:\\Users\\bgree\\Documents\\capstone\\Eng\\exported-features\\" + targetGroup + "\\fileList.txt"), "utf-8"))) {
							writer.write(write + "\n");		
			   } catch(IOException e) {
			...
			// Evaluate
			double[] performance=EvaluateClassifier(Classifier,groupDir + targetGroup +"_prob.csv");
			String resultsPath = "C:\\Users\\bgree\\Documents\\capstone\\Results\\Logistic Regression\\2-23-18.txt";

			try {
				FileWriter fstream = new FileWriter(resultsPath, true);
				BufferedWriter out = new BufferedWriter(fstream);
				out.write("Group "+targetGroup+", Accuracy: "+performance[0] + "\n"); 
				out.write("0 class ("+performance[7]+"/"+performance[8]+"/"+performance[9]+"): Precision="+performance[4]+",Recall="+performance[5]+",F1-Measure="+performance[6] + "\n");
				out.write("1 class ("+performance[10]+"/"+performance[11]+"/"+performance[12]+"): Precision="+performance[1]+",Recall="+performance[2]+",F1-Measure="+performance[3] + "\n\n");
				out.close();
				//System.out.println(FileName+" Classifier Saved!");
			} catch (Exception e) {
				e.printStackTrace(); 
